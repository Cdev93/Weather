...
const mapWeatherData = (info) => {
  ...
  if (!data || !data.main || !data.weather || !data.sys)
    return console.warn('Error no data')

  if (!data2) return console.warn('Error no data2')
  ...
  const obtainLocalTime = (timezone) => {
    const localTime = DateTime.fromMillis(Date.now(), {
  ...
  const getNextHours = () => {
    const forecast = data2.list.filter((prev) => {
      const hourPrev = formatDate(
        new Date(prev.dt_txt).toLocaleString(DateTime.TIME_SIMPLE),
      )
