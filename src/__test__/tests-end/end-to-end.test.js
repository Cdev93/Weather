...
test.describe('Testing Searchs & adds', () => {
  ...
  test('Debería realizar el procedimiento esperado de una búsqueda con su correspondiente almacenado y eliminación', async ({
    page,
  }) => {
    ...
    await expect(page.locator('button.sugg-btn')).toBeVisible({timeout:30000})
    ...
    await page
      .locator(
        'div.styles-module_carousel-base__3keqD > div:nth-child(1) > button:nth-child(1)',
      )
      .click()
    await page
      .locator(
        'div.styles-module_carousel-base__3keqD > div:nth-child(3) > button:nth-child(1)',
      )
      .click()
...
test.describe('Testing not found', () => {
  test('Deberían mostrarse el fondo de "Not found" al introducir una búsqueda no válida', async ({
    page,
  }) => {
    //comprobamos que la barra de búsqueda está vacía
    await expect(page.locator('input.input-container-sb')).toBeEmpty()
