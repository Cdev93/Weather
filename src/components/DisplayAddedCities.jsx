...
function DisplayCities(props) {
    ...
    const [controlLoad, setControlLoad] = useState(false);
        ...
        const handleWeatherSearch = async() => {
            ...
            const mappedCities = cities.map((city)=>{return city.name})
            let dataMapped = []
    ...
    const handleChanges = async(id, city) => {
            ...
            if(props.visible && (props.controlCity.toLowerCase() == city.toLowerCase())){
    ...
    return (
        <> 
          ...
          {weatherData && weatherData.length!=0  ? ( <div className="added-cities-cont">
                <h2>Saved Cities</h2>
                ...
                <div className="grid-column" key={city.id}>
                    ...
                    <button onClick={(e) => handleChanges(city.id, city.city) && e.stopPropagation()} id="delete-btn-added"><img src={`src/assets/weather_icons/eliminar.png`} alt="delete-icon"></img></button>
                        ...
                        <div className="widget-bottom">
                            <p>{city.time}</p><p>{city.tem_min}º{city.tem_max}º</p>
                        </div>
